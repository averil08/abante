...
const Dashboard = () => {
  ...
  const handleCancel = () => {
    ...
    if (currentPatientForCancel && currentPatientForCancel.id) {
      cancelPatient(currentPatientForCancel.id);
    } else {
  ...
  const cancelPatients = patients.filter(p => {
    if (p.isInactive) return false;
    // Show cancelled patients regardless of `inQueue` flag (they are marked `inQueue: false` on cancel)
              ...
              <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    onClick={async () => {
                      const currentPatientForCancel = patients.find(p => p.queueNo === currentServing);
                      if (currentPatientForCancel && currentPatientForCancel.id) await cancelPatient(currentPatientForCancel.id);
                      // small delay to let state update and then advance
                      if (typeof callNextPatient === 'function') {
